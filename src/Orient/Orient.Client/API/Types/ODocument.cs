...
namespace Orient.Client.API.Types
{
    ...
    {
        private static readonly ConcurrentDictionary<Type, bool> ImplementsMap = new ConcurrentDictionary<Type, bool>();

        #region Properties which holds orient specific fields

        public ORID ORID {
            get { return HasField("@ORID") ? GetField<ORID>("@ORID") : null; }
        ...
        public T GetField<T>(string fieldPath) {
            ...
            if (TryGetValue(fieldPath, out fieldValue)) {
                ...
                if (!type.IsPrimitive && ImplementsIList(type) && !type.IsArray) {
                    ...
                    var enumerator = EnumerableFromField(fieldValue).GetEnumerator();
                ...
                if (type.Name == "HashSet`1") {
                    ...
                    var enumerator = ((IEnumerable) fieldValue).GetEnumerator();
                    ...
                    while (enumerator.MoveNext()) {
                        ...
                        if (enumerator.Current is ODocument) {
                            ...
                            addMethod.Invoke(value, new[] {enumerator.Current});
                        }
                    }
                    return value;
                }
            ...
            if (fieldPath.Contains(".")) {
                ...
                foreach (var field in fields) {
                    ...
                    if (embeddedDocument.ContainsKey(field)) {
                        ...
                        var tempDocument = new ODocument();
        ...
        public T ToUnique<T>(ICreationContext store) where T : class {
            ...
            var genericObject = (T) store.CreateObject(OClassName);
